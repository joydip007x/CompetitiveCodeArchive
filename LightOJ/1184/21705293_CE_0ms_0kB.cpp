      ...
      for(int cas=1; cas<=t ; cas++ ){
             ...
             for(int i=0; i<dm.size() ; i++){
             	 for(int j=0; j<dw.size() ; j++){
                       ...
                       if( abs(dm[i].second.second-dw[j].second.second)>5  || abs(dm[i].second.first-dw[j].second.first)>12 )  continue;
             ...
             for(int i=0; i<sm.size() ; i++){
             	 for(int j=0; j<sw.size() ; j++){
                       ...
                       if( abs(sm[i].second.second-sw[j].second.second)>5  ||  abs(sm[i].second.first-sw[j].second.first)>12 )  continue;
             ...
             mxflw=0;
             while(bfs(0,w+m+1)){
               	 for(int i=w+m+1; i!=0 ; i=par[i]) g[par[i]][i]-=1,g[i][par[i]]+=1;
                 mxflw++;
             }
         	 printf("Case %d: %d\n",cas,mxflw);
      }
