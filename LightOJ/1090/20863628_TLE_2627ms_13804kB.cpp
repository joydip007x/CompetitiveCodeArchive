...
void factorize(ll a,ll b){
   ...
   for(ll i=0; prime[i]<=a and prime[i]<=11; i++){
        ll cnt=0;
...
int main()
{
  ...
  for(int cas=1 ; cas<=q ; cas++){
       ...
       sn=n-r;
       factorize(p,q);
      for(int i=0 ; prime[i]<=n and  prime[i]<=11 ; i++){
          int cur=prime[i],cnt=0,p=prime[i];
          while(n/p)cnt+=(n/p),p=p*cur;
          m[prime[i]]+=cnt;
      }
      for(int i=0 ; prime[i]<=r and prime[i]<=11 ; i++){
          int cur=prime[i],cnt=0,p=prime[i];
          while(r/p)cnt+=(r/p),p=p*cur;
          m[prime[i]]-=cnt;
      }
      for(int i=0 ; prime[i]<=sn and prime[i]<=11 ; i++){
          int cur=prime[i],cnt=0,p=prime[i];
