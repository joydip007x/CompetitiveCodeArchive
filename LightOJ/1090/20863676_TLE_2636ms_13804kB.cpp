...
void factorize(ll a,ll b){
   ...
   for(ll i=0; prime[i]<=a and prime[i]<=7; i++){
        ll cnt=0;
        while(a%prime[i]==0) a=a/prime[i],cnt++;
        m[ prime[i] ]=cnt*b;
   }
   if(a%2==1 and vis[a]==0)m[a]=1*b;
   return;
}
int main()
{
  ...
  for(int cas=1 ; cas<=q ; cas++){
       ...
       sn=n-r;
       factorize(p,q);
      for(int i=0 ; prime[i]<=n and  prime[i]<=7 ; i++){
          int cur=prime[i],cnt=0,p=prime[i];
          while(n/p)cnt+=(n/p),p=p*cur;
          m[prime[i]]+=cnt;
      }
      for(int i=0 ; prime[i]<=r and prime[i]<=7 ; i++){
          int cur=prime[i],cnt=0,p=prime[i];
          while(r/p)cnt+=(r/p),p=p*cur;
          m[prime[i]]-=cnt;
      }
      for(int i=0 ; prime[i]<=sn and prime[i]<=7 ; i++){
          int cur=prime[i],cnt=0,p=prime[i];
