...
#define fast                    ios_base::sync_with_stdio(false);cin.tie(NULL);cin.out(NULL);
...
void xdfs(ll x,ll y, ll vis[ ][sz] )
{
   ...
   {
       ll tox=now1+movx[xx] , toy=now2+movy[xx];
       if (  chk(tox,toy ) &&  orivs[ tox][toy ] ==-1)
                  {
                      vis[tox][toy]=min( vis[tox][toy],vis[now1][now2]+1);
                     q.push(tox);q.push(toy);
                     orivs[tox][toy]=1;
                }
   }
...
//                cout<<"in - ["<<i<<"]["<<j<<"]  fire came at = "<<fvis[i][j]<<" second ";eee;
     ...
     cout<<"Case "<<h+1<<": ";
     if(ansx!=0) {cout<<*min_element(all(pp))+1<<endl; }
