...
void update(int node, int start, int end, int idx, int val){
    ...
    {
        arr[idx]=val;
        tree[node][0]=tree[node][1]=tree[node][2]= val;
        return;
    }
    else
    {
...
int query(int node, int start, int end, int l , int r){
      ...
      if(r<start or end<l or end<start ) return 0;
...
int queryMIN(int node, int start, int end, int l , int r){
      ...
      if(r<start or end<l or end<start ) return LLONG_MAX;
...
int queryMAX(int node, int start, int end, int l , int r){
      ...
      if(r<start or end<l or end<start) return LLONG_MIN;
...
int32_t main()
{
  ...
  cin>>n>>q;
 /// std::vector<ll> tt={0,LLONG_MAX,LLONG_MIN};
  std::vector<ll> tt={0,0,0};
  for(int i=0;i<=(4*N)+10; i++) tree[i]=tt;
