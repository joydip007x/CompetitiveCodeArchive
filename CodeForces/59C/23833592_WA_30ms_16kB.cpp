...
int main()
{
     ...
     ios_base::sync_with_stdio(false);cin.tie(nullptr); cout.tie(nullptr); 
   	 ...
   	 for(int i='a';i<='z'&& i<=stp ;i++) if(!m[i] && i!=q )v.pb(i);
   	 ...
   	 ll mid=0;
   	 if(cnt1%2 ) {mid=1,cnt1--;}
     
   	 /////cout<<cnt1<<" | "<<s<<" V: "<<v.size()<<endl;

   	 for(int i=0,j=s.size()-1; i<j && cnt1 ; i++,j--){

         if(s[i]!=q && s[j]!=q) continue;
         if((s[i]==q && s[j]!=q) || (s[i]!=q && s[j]==q))continue;

        /////// cout<<" C> "<<cnt1<<":"<<cnt1/2<<" > "<<v.size()<<endl;

         if(cnt1==2 && v.size()==1 && mid){
         ...
         else if(cnt1==1 && v.size()==1 && mid){
 	    	 s[s.size()/2]=v[0];
 	    	 mid=0;
 	    	 break;
         }
         else if( (cnt1/2) > v.size() ){
