...
string a,b;
ll base= 100003,  mod= 100019, bas;
ll base2=100043, mod2= 100049, bas2;
...
int main()
{
  ...
  for(int i=0 ; i<b.size() ; i++){
    ...
    ass=(ass+(b[i]*bas )+mod )%mod;
    ass3=(ass3+(b[i]*bas2)+mod2)%mod2;
  }
  u[ass]=u[ass3]=1;
  bas=1; ass=0;
  bas2=1; ass3=0;
  for(int i=0 ; i<a.size() ; i++ ){
    ...
    bas=(bas*base)%mod;
    bas2=(bas2*base2)%mod2;
    ass=(ass+(a[i]*bas)+mod)%mod;
    ass3=(ass3+(a[i]*bas2)+mod2)%mod2;
  }
  ass2=ass;
  ass4=ass3;
  ...
  for(int i=a.size()-1, j=1 ;  i>=0 ; i--,j++){
      ...
      if(u[ass2] && u[ass4])  return cout<<j-1<<endl,0;
