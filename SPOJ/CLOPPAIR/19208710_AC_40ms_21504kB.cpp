...
double extendedclos(ll a, ll b, double midxdis ,double mn){
...
void ysort(ll a ,ll b){
 ...
 double clospair(ll a, ll b){
  ...
  ll mid=(a+b)/2;
  Point pmid=p[mid];//this line took my 8 hours..beware in recursion , while u  use GLOBALLY    DECLARED  shits and use thm inside  recursion :) 
   double mina=clospair(a, mid);
   double minb=clospair(mid+1, b);
   double mind=min(mina,minb);
   ysort(a,b);
  return extendedclos(a,b, pmid.x ,mind) ;
}
