...
long double ans1=LLONG_MAX*1.00,ans;
struct Point
{
    ll x,y ;
};
...
long double extendedclos(ll a, ll b, long double mn){

    Point tmp[sz];
    mid=p[(int)((a+b)/2)]; ll i=a,index=0,j=b;
    while(i<=j){
        if(   abs(p[i].y-mid.y)<mn ) tmp[index++]=p[i];
        i++;
    }
    long double tmpmn=LLONG_MAX*1.00;
    for(i=0 ; i<index ; i++)
    {
...
long double clospair(ll a, ll b){

  if(a==b) return INT_MAX*1.00;
  ll mid=(a+b)/2;
  long double mina=abs(clospair(a, mid));
  long double minb=abs(clospair(mid+1, b));
  long double mind=min(mina,minb);
  ysort(a,b);
