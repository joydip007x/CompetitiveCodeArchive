...
#define ll                        int
#define sz 50000+10
ll n,ansa=-1,ansb=-1,unvis=0;
double ans1=2147483648.00,ans;
struct Point
{
    double x,y ;
};
Point p[sz], px,qx, tmp[sz],stp[sz];
...
double extendedclos(ll a, ll b,ll mid,  double mn){
    ll i=a,index=0,j=b;
    while(i<=j){
        if( abs(p[i].x-mid)<=mn ) stp[index++]=p[i];
        i++;
    }
    long double tmpmn=LLONG_MAX*1.00;
    for(i=0 ; i<index ; i++)
    {
        for(ll j=i+1; j<index  and abs( stp[i].y-stp[j].y )<mn ; j++){
             double lowclas= dist( stp[i],stp[j]  );
            if(ans1>=lowclas){
                px=stp[i] , qx=stp[j];
                ans1=lowclas;
            }
...
void ysort(ll a ,ll b){
 ...
 //  ysort(a,b);
  return extendedclos(a,b,mdx,mind) ;
}
int main(){
