        ...
        vector < ll >ass; bool flag=false;
         for (    ll  pp=0      ;  pp<prime.size()  ; pp++)
         {
            if( CmpMAP1[   prime[pp]   ]   >CmpMAP2 [ prime [ pp] ]   ) {flag=true;}
             if(CmpMAP1[   prime[pp]   ]!=0 &&  CmpMAP2[   prime[pp]   ] !=0    & flag==false  )/////at least the prime should be used in organizing the number  otherwise its not even in the prime factorization for sure ;
                    {          ass.pb(   CmpMAP2[ prime[pp] ] /CmpMAP1 [ prime [ pp] ]  );       }           
         }
         srt(ass);
         ll ans=0;
...
/*optional solution -
https://github.com/lamphanviet/competitive-programming/blob/master/uva-online-judge/accepted-solutions/10780%20-%20Again%20Prime%20No%20Time.cpp
*/