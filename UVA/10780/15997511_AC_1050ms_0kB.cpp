    ...
    seive() ;
     ...
     ll tcas ,n,m;
      ...
      for ( ll k=1 ; k<=tcas ; k++)
      {
        ...
        CmpMAP2=Fact [ m ];
//         for ( auto it : CmpMAP1)
//         {if(it.second>0) cout<<it.first<< " = "<<it.second<<" ";eee;
//
//         }
//         eee;
//        for ( auto it : CmpMAP2)
...
//        eee;
        ...
        vector < ll >ass; bool flag=false;
         ...
         {
             ll XC = min (  CmpMAP1[   prime[pp]   ]   , CmpMAP2 [ prime [ pp] ]      );
             if( CmpMAP1[   prime[pp]   ]   >CmpMAP2 [ prime [ pp] ]   ) {flag=true;}
             if(XC>0  &&  CmpMAP1[   prime[pp]   ]!=0 &&  CmpMAP2[   prime[pp]   ] !=0  && CmpMAP2[ prime[pp ] ] >=CmpMAP1[prime[pp] ]   ) {ass.pb(   CmpMAP2[ prime[pp] ] /CmpMAP1 [ prime [ pp] ]           ); }
         }
         srt(ass);
         ll ans=0;
