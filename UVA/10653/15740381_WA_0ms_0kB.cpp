...
{
    if ( nx>=0 && nx<m && ny>=0 && ny<n && cell[nx][ny]!=-1 ) return true;
   else  return false;
}
/////////////////////////////////////////////////////////////ACCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCc

int main ()
{
      fast;
    while(bool (cin>>m>>n)==true  )
    {
        if(m==0 && n==0) {break;}
       /// ll cell[m][n]; ll d[m][n]; ll vis[m][n];
...
void  bfs ( ll startx , ll starty  )
{
         ...
         while(!Q.empty())
         {
             ...
             for(ll cir=0; cir<4; cir++)
             {
                 ...
                 if(   validx(topx,topy)==true && vis[topx][topy]==0   /*!d[topx][topy] */ )
                 {
                     vis[topx][topy]=1;
