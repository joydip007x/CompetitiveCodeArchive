...
int bfs( ll z, ll x,ll y )
{
   ...
   while(!q.empty())
   {
       ...
       //// RIGHT LEFT  UP DOWN 
            for (int i=0 ; i<4 ; i++)
            {
                int cz=tpz, cx=tpx+mvx[i],cy=tpy+mvy[i];
            ...
            ////from STATIC POSITION to AVOBE block or BELOW LVL blocks;;;
          //  just  fron his position to one lvl up (means x,y remains same ) .. intotal 6 combination moves possilbe ;
          /// 0, 0, 0, to 1,1, 0 impossible , 1,1,1, to 2,1,1 , or 0,1, 1, possible ;(
            for(int k=0 ; k<2 ; k++)
            {
                int cz=tpz+dx3[k], cx=tpx,cy=tpy;
                if(   valid(cz,cx,cy) and vis[cz][cx][cy]==-1  )
                {
                    q.push(cz),q.push(cx),q.push(cy);
                    vis[cz][cx][cy]=vis[tpz][tpx][tpy]+1;
