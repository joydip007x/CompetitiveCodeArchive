   ...
   while( q.size() ){
      ...
      pp a=q.front();q.pop();
       ...
       for(int i=0 ; i<=3 ; i++){
            ...
            if(  dis[mc][ma][mb] > dis[mc][a.first.second][a.second]+1 ){

              dis[mc][ma][mb] = dis[mc][a.first.second][a.second]+1;
              q.push( { {mc,ma} ,mb } );
             }
...
int main(){
     ...
     while( scanf("%d %d %d",&l,&w , &h ) && l!=0 && h!=0  && w!=0 ){
        ...
        for(int kk=0 ; kk<h; kk++){
         for(int i=0 ; i<l; i++){
            ...
            for(int j=0 ; j<strlen(s) ; j++) {
                ...
                dis[kk][i][j]=INT_MAX-100;

            }
         }
        }
         ll res=bfs(sh,sr,sc);
         if(res==INT_MAX-100) printf("-1\n");
         else printf("%d\n",res);
