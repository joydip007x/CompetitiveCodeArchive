...
ll LIS(ll las, ll cu)
{
    ...
    if( p[cu].sc.fs < p[las].sc.fs   ) ret dp[las][cu]=max( 1+LIS(cu,cu+1) , LIS(las,cu+1)   );
...
int main()
{
    ll i=1,xxx,pf=0; memset(dp,-1,sizeof dp);
    p[0]={  -1  , { INT_MAX,-1 } };
    ...
    {
        if( 1+dp[cur][cur+1] > dp[las][cur+1] and p[las].sc.fs>p[cur].sc.fs    )
        {
            printf("%d\n",p[cur].sc.sc);
            las=cur; xxx--;
        }
