#include<bits/stdc++.h>
using namespace std ;

#define ll            long long
#define pb            push_back
#define nikal_laura   ios::sync_with_stdio(false); cin.tie(nullptr);cout.tie(nullptr);
#define sc(c)          scanf("%d",&c);
ll a[100000+20],n;
void printsol(){

    cout<<"YES\n";
    for(int i=0; i<n; i++) {
        cout<<a[i]<<" ";
    }
    cout<<"\n";
    return;
}
int main(){

    ll t;
    cin>>t;
    while(t--){

        ll k,start,f=0 , prob=0 , cnt=0, jk=0;
        cin>>n>>k;
        memset(a,-1,sizeof a);
        start=-1; ll pos=0;
        for(int i=0 ; i< n; i++){
             cin>>a[i];
             if(!pos && a[i]!=-1) { start=i; pos=1;}
             if(a[i]==-1) cnt++;
        }
        if( cnt==0){   printsol();continue;     }
        if( cnt==n ){
          cout<<"YES\n";
          for(int i=0; i<n; i++)
                 (i%2==0) ? cout<<1<<" ":cout<<2<<" ";
          cout<<"\n";
          continue;
        }
        if(k==2){
             for(int i=start-1 ; i>=0 ; i--){
                  if(a[i]==-1){
                    if(a[i+1]==1) a[i]=2;
                    else a[i]=1;
                }
               if(a[i]==a[i+1] || (i-1>=0 && a[i]==a[i-1] )) f=1;
             }
             for(int i=start+1; i+1<n && f==0 ; i++){
                if(a[i]==-1){
                    if(a[i-1]==1 ){
                        if(a[i+1]==2) { f=1; break;}
                        a[i]=2;
                    }
                    else if(a[i-1]==2 ){
                        if(a[i+1]==1) { f=1; break;}
                        a[i]=1;
                    }
                }
              if(a[i]==a[i+1] || (i-1>=0 && a[i]==a[i-1] )) f=1;
             }
             if(f || a[n-1]==a[n-2]) { cout<<"NO\n"; continue;}
             if(a[n-2]==1 && a[n-1]==-1)a[n-1]=2;
             else if(a[n-1]==-1 && a[n-2]==2 ) a[n-1]=1;
             printsol();
             continue;
        }
        for(int i=start-1 ; i>=0 ; i--){
                  if(a[i]==-1){
                    if(a[i+1]==1) a[i]=2;
                    else a[i]=1;
                }
             }
             for(int i=start+1; i+1<n && f==0 ; i++){
                if(a[i]==-1){
                    if(a[i-1]==1 ){
                        if(a[i+1]==2)  a[i]=3;
                        else if(a[i+1]==-1) a[i]=3;
                        else if( a[i+1]==1) a[i]=2;
                        else  a[i]=2;
                    }
                    else if(a[i-1]==2 ){
                        if(a[i+1]==1) a[i]=3;
                        else if(a[i+1]==-1) a[i]=3;
                        else if(a[i+1]==2)  a[i]=1;
                        else  a[i]=1;
                    }
                    else{
                        if(a[i+1]==1) a[i]=2;
                        else if(a[i+1]==-1) a[i]=2;
                        else if(a[i+1]==2)  a[i]=1;
                        else  a[i]=1;
                    }
                }

             }
             if(a[n-1]==-1){
                 if(a[n-2]==1) a[n-1]=2;
                 else if(a[n-2]==2) a[n-1]=1;
                 else a[n-1]=2;
             }
             printsol();
             continue;
    }
}
