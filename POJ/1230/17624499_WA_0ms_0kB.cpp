...
ll n,k,wals[sz],wale[sz],rx=mod , ex=0LL,curmax=0LL,
...
void countosum(ll x)
{
   ...
   loopeq(i,0,100) osum=max(osum,m[i]);
    wale[x]=wals[x]=-1;
}
...
void pre()
{
    ...
    m.clear();
    rx=inf , ex=-1LL,curmax=-1LL,out=0LL, osum=-1LL;
}
int main()
{
  o(t);
  while(t--)
  {
        ...
        loop(i,0,n)
        {
            ...
            wals[i]=min(x1,x2);
            wale[i]=max(x1,x2);
            rx=min(min(min(rx,x1),x2),ex);
            ex=max(max(max(ex,x2),x1),rx);
        }
        if(!k) { printf("%lld\n",n);continue ;}
        if(k>n || osum<=k ){ printf("0\n");     continue ;}
        while( osum>k )
        {
            curmax=-1LL;removewal=-1LL;
            for( ll i=0 ; i<n ; i++)
            {
                if(wale[i]<0 || wals[i]<0) continue;
                ll xx=procesthewall(i);
                if( xx>=1 and xx>curmax) {removewal=i ; curmax=xx;}
            }
            if(removewal==-1 || curmax==-1 ) break;
            out++;
            countosum(removewal);
        }
        printf("%lld\n",out); out=0;
